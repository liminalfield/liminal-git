 ...
 */
export type GitErrorCode = 'REPOSITORY_NOT_FOUND' | 'REPOSITORY_CORRUPTED' | 'INVALID_REPOSITORY' | 'FILE_NOT_FOUND' | 'FILE_NOT_IN_REPOSITORY' | 'PATH_TRAVERSAL' | 'NOTHING_TO_COMMIT' | 'MERGE_CONFLICT' | 'UNCOMMITTED_CHANGES' | 'UNSTAGED_CHANGES_WOULD_BE_LOST' | 'DETACHED_HEAD' | 'BRANCH_NOT_FOUND' | 'BRANCH_ALREADY_EXISTS' | 'CANNOT_DELETE_CURRENT_BRANCH' | 'BRANCH_NOT_MERGED' | 'TAG_NOT_FOUND' | 'TAG_ALREADY_EXISTS' | 'INVALID_PATH' | 'INVALID_COMMIT_HASH' | 'INVALID_BRANCH_NAME' | 'INVALID_TAG_NAME' | 'IO_ERROR' | 'GIT_OPERATION_FAILURE' | 'CONFIG_MISSING';
/**
 ...
 */
export declare function isFileNotFoundError(error: StructuredGitError<any>): error is FileNotFoundError;
/**
 * Type guard: Check if error is a branch-not-found error
 */
export declare function isBranchNotFoundError(error: StructuredGitError<any>): error is BranchNotFoundError;
/**
 * Type guard: Check if error is a merge conflict error
 */
export declare function isMergeConflictError(error: StructuredGitError<any>): error is MergeConflictError;
/**
 * Type guard: Check if error is an invalid path error
 */
export declare function isInvalidPathError(error: StructuredGitError<any>): error is InvalidPathError;
/**
 * Type guard: Check if error is an unstaged-changes-would-be-lost error
 */
export declare function isUnstagedChangesWouldBeLostError(error: StructuredGitError<any>): error is UnstagedChangesWouldBeLostError;
/**
 * Type guard: Check if error is a config-missing error
 */
export declare function isConfigMissingError(error: StructuredGitError<any>): error is ConfigMissingError;
/**
